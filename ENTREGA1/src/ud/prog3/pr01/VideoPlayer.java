...
public class VideoPlayer extends JFrame {
	...
	public VideoPlayer() {
		...
		bAnyadir.addActionListener( new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				ficheros = JOptionPane.showInputDialog( "Introduce el nombre de los ficheros" );
				// ficheros = ...
	...
	private void lanzaVideo() {
		if (mediaPlayerComponent.mediaPlayer()!=null &&
			...
			// Paso 9
			long fecha = ficVideo.lastModified();
			DateFormat formatoFecha = DateFormat.getDateTimeInstance(DateFormat.SHORT, DateFormat.SHORT);
			String fechaString = formatoFecha.format(new Date(fecha));
			lMensaje.setText( "Última vez modificado: " + fechaString );
			//
	...
	public static void main(String[] args) {
		...
		if (args.length < 2) {
			...
			ficheros = JOptionPane.showInputDialog( "Introduce el nombre de los ficheros (con comodines)" );
			// ficheros = ???
		} else {
			ficheros = args[0];
			path = args[1];
